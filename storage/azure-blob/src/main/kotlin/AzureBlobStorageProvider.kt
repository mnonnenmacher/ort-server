...
class AzureBlobStorageProvider(private val blobContainerClient: BlobContainerClient) : StorageProvider {
    ...
    override fun contains(key: Key): Boolean = blobContainerClient.getBlobClient(key.key).exists()

    override fun delete(key: Key): Boolean = blobContainerClient.getBlobClient(key.key).deleteIfExists()
}
